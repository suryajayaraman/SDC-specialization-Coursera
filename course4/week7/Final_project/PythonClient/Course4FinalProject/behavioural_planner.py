...
class BehaviouralPlanner:
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        for index in range(wp_index + 1, total_waypoints):
            arc_length += np.sqrt(np.square(waypoints[index][0] - waypoints[index-1][0]) + np.square(waypoints[index][1] - waypoints[index-1][1]))
            if arc_length > self._lookahead:
                break
        return index
...
def get_closest_index(waypoints, ego_state):
    ...
    for i in range(len(waypoints)):
        waypt_dist = np.sqrt(np.square(ego_state[0] - waypoints[i][0]) + np.square(ego_state[1] - waypoints[i][1]))
        if waypt_dist < closest_len:
            closest_len = waypt_dist
            closest_index = i
