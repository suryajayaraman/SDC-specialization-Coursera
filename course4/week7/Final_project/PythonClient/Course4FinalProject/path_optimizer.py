...
class PathOptimizer:
    ...
    def optimize_spiral(self, xf, yf, tf):
        ...
        # variables for the optimizer are set by p0. You should use the `
        # optimization methods.
        # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
        # ------------------------------------------------------------------
        res = scipy.optimize.minimize(self.objective, x0 = p0, method='L-BFGS-B', jac=self.objective_grad,
                                      bounds = bounds)
        # ------------------------------------------------------------------
    ...
    def thetaf(self, a, b, c, d, s):
        ...
        thetas = np.multiply(a,s) + \
                 np.multiply(np.divide(b,2.0),np.power(s,2)) + \
                 np.multiply(np.divide(c,3.0),np.power(s,3)) + \
                 np.multiply(np.divide(d,4.0),np.power(s,4))
        return thetas
        # ------------------------------------------------------------------
