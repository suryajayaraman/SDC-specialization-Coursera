...
class LocalPlanner:
    ...
    def get_goal_state_set(self, goal_index, goal_state, waypoints, ego_state):
        ...
        if goal_index == len(waypoints)-1:
            point1_index = goal_index - 1 
            point2_index = goal_index 
        elif goal_index >=0:
            point1_index = goal_index
            point2_index = goal_index + 1
        
        delta = np.subtract(waypoints[point2_index],waypoints[point1_index])
        #global yaw of the goal state / next point
        ...
        '''
        You need to know the pose of Entity A in the global space (x1,y1,θ), 
        where θ is the orientation relative to the x axis.
        ...
        # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
        # ------------------------------------------------------------------
        goal_x =  (goal_state_local[0] - ego_state[0] ) * cos(ego_state[2]) + \
                  (goal_state_local[1] - ego_state[1] ) * sin(ego_state[2])
                  
        goal_y = -(goal_state_local[0] - ego_state[0] ) * sin(ego_state[2]) + \
                  (goal_state_local[1] - ego_state[1] ) * cos(ego_state[2])
        # ------------------------------------------------------------------
        ...
        for i in range(self._num_paths):
            ...
            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
            # ------------------------------------------------------------------
            x_offset = np.multiply(offset, cos(goal_t + pi/2))
            y_offset = np.multiply(offset, sin(goal_t + pi/2))
            # ------------------------------------------------------------------
